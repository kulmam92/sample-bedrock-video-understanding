...
class VideoUpload extends React.Component {
    ...
    generatePresignedUrls (file) {
        ...
        FetchPost("/util/manage-s3-presigned-url", {"FileName": file.name, "NumParts": numChunks, "Action": "create"}, "TLabsService")
            .then((data) => {
                  //console.log(resp);
    ...
    async submitTask (urlResp) {
        ...
        const { username } = getCurrentUser().then((username) => {
            ...
            // Start task
            FetchPost('/tlabs/embedding/start-task', payload, "TLabsService")
                .then((data) => {
    ...
    async uploadFile(urlResp) {
        ...
        if (urlResp.uploadPartUrls === null || urlResp.uploadPartUrls.length === 0) return;
            ...
            await Promise.all(uploadPromises).then(() => {
                ...
                FetchPost("/util/manage-s3-presigned-url", payload, "TLabsService")
                .then((result) => {
