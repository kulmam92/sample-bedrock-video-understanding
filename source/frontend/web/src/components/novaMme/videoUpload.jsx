...
import VideoMmEmbeddingSetting from './videoMmEmbedSetting'
...
class VideoUpload extends React.Component {
    ...
    handleNavigate = e => {
        if(e.detail.requestedStepIndex === 1) {
            ...
            this.setState({taskName: this.state.uploadFiles[0].name});
        }
        else if(e.detail.requestedStepIndex === 2) {
            this.setState({alert: null, request: this.mmEmbedSettingRef.current.getRequest()})
        }
    ...
    generatePresignedUrls (file) {
        ...
        FetchPost("/util/nova-srv-manage-s3-presigned-url", {"FileName": file.name, "NumParts": numChunks, "Action": "create"}, "NovaService")
            .then((data) => {
                  //console.log(resp);
    ...
    async submitTask (urlResp) {
        ...
        const { username } = getCurrentUser().then((username) => {
            ...
            // Start task
            FetchPost('/nova/embedding/start-task', payload, "NovaService")
                .then((data) => {
    ...
    async uploadFile(urlResp) {
        ...
        if (urlResp.uploadPartUrls === null || urlResp.uploadPartUrls.length === 0) return;
            ...
            await Promise.all(uploadPromises).then(() => {
                ...
                FetchPost("/util/nova-srv-manage-s3-presigned-url", payload, "NovaService")
                .then((result) => {
    ...
    handelFileChange = (e) => {
        const supportedFormats = ["mp4", "mov", "mkv", "webm", "flv", "mpeg", "mpg", "wmv", "3gp"];
        const filteredFiles = [];
        const invalidFiles= [];
        ...
        this.setState({
            ...
            alert: invalidFiles.length === 0?null: `File(s) "${invalidFiles.join(', ')}" not in supported format (${supportedFormats.join(', ')}) and has been removed.`
        });
