...
def lambda_handler(event, context):
    ...
    while start_ts <= duration:
        chunks.append({
            ...
            "task_id": task_id,
            "similarity_method": event["Request"]["PreProcessSetting"].get("SimilarityMethod")
        })
    ...
    while retries < max_retries:
        try:
            ...
            if image_s3_bucket and image_s3_key:
                image_format = image_s3_key.split('.')[-1].lower()
                      ...
                      "inputSchema": {
                        "json": {
                          ...
                          "properties": {
                            "result": {
                              "type": "boolen",
                              "description": "If suitable to be used as a video thumbnail. It should not be a solid or blank screen, must have sufficient contrast and clarity."
                            }
                          },
                          "required": [
                            "result"
...
def update_usage_to_db(task_id, index, name, model_id, input_tokens, output_tokens, total_tokens):
    usage = {
        "id": f"{task_id}_{index}_{name}_frame_thumb",
        "index": index,
